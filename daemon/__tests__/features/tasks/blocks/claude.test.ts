...
import type { ClaudeNode } from '@daemon/types/task/schema'
import { cleanup, tempDir } from '@daemon/test'
import { claudeBlock } from '@daemon/features/tasks/blocks/claude'
...
it('hands the session literal paths and leave to edit its workspace', async () => {
  ...
  await claudeBlock.run(nodeAt(), ctx)
...
it('drops the file talk and the scratch grant when there is no scratch', async () => {
  ...
  const result = await claudeBlock.run(nodeAt(), ctx)
  const args = await argsOf(dir)
