...
import { commentBlock, pullBlock } from '../../../src/tasks/blocks/github'
import type { GithubReach, StepCtx } from '../../../src/tasks/blocks/core'
import { writeGithubTarget } from '../../../src/tasks/scratch'
...
it('says what the step before it wrote, on the issue the run is about', async () => {
  ...
  expect(await commentBlock(comment(), ctx)).toEqual({
...
it('prefers what the node was told over what the run was about', async () => {
  ...
  await commentBlock(comment({ repo: 'you/other', number: 12 }), ctx)
...
it('stops rather than guessing which issue it meant', async () => {
  ...
  await expect(commentBlock(comment(), ctx)).rejects.toThrow(/which issue/)
...
it('stops where the step before it said nothing at all', async () => {
  ...
  await expect(commentBlock(comment({ number: 7 }), ctx)).rejects.toThrow(/nothing to say/)
})

it('needs a connection, and says which one is missing', async () => {
  const ctx = await ctxAt()

  await expect(commentBlock(comment({ number: 7 }), ctx)).rejects.toThrow(
    /connect GitHub in Settings/,
  )
  await expect(pullBlock(pull(), ctx)).rejects.toThrow(/connect GitHub in Settings/)
})
...
it('opens a pull request titled by the first line of what it was handed', async () => {
  ...
  expect(await pullBlock(pull(), ctx)).toEqual({
...
it('refuses to open a branch against itself', async () => {
  ...
  await expect(pullBlock(pull(), ctx)).rejects.toThrow(/nothing to open/)
})
...
it('stops where the checkout has no GitHub remote and the node names none', async () => {
  ...
  await expect(pullBlock(pull(), ctx)).rejects.toThrow(/which repository/)
})
