...
import type { MuseNode } from '@broodmother/types/task/schema'
import { cleanup, tempDir } from '../../../src/test'
import type { StepCtx } from '../../../src/tasks/blocks/core'
import { museBlock } from '../../../src/tasks/blocks/muse'
...
it('drops the file talk when there is no scratch', async () => {
  ...
  const result = await museBlock(nodeAt(), ctx)
  const args = await argsOf(dir)
