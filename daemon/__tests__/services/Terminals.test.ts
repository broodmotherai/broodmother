...
import { cleanup, delay, fakeCrontab, tempDir, until } from '@daemon/test'
import { seedSkills } from '@daemon/utils/skills'
...
describe('terminals', () => {
  ...
  it('follows the project you switch to', async () => {
    ...
    await handle.context.workspace.openProject(other)
  ...
  it('opens in the repo when one is open', async () => {
    const handle = await server()
    const repo = await handle.context.workspace.addRepo({ name: 'api' })
