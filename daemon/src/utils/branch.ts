export { PRIMARY } from '@daemon/constants/files'
import { mkdir, readdir, rm, stat } from 'node:fs/promises'
import path from 'node:path'
import { Git } from './git'
import { nameProblem } from './path'
import { branchNameProblem } from '@daemon/types/branch'
import type { Branch } from '@daemon/types/branch'
import { AppError } from '@daemon/types/error'

export type { Branch } from '@daemon/types/branch'
...
/** How a repo's open checkout is keyed in the config: one project may link many repos, and
 *  each of them stands on a branch of its own. */
export const branchKey = (project: string, repo: string) => `${project}#${repo}`
