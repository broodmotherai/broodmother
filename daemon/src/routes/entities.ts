import { actor, parse } from './request'
...
export const entities = {
  ...
  'POST /api/entities': async (c, ctx) =>
    c.json(await ctx.entities.record(await parse(c, newEntityBody), actor(c))),
  ...
  'POST /api/entity/link': async (c, ctx) => {
    ...
    return c.json(await ctx.entities.link(path, relation, target, actor(c)))
  },
