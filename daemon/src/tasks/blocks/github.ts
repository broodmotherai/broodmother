...
import type { GithubCommentNode, GithubPullNode } from '@broodmother/types/task/schema'
import { readGithubTarget } from '../scratch'
import type { GithubReach, StepCtx, StepResult } from './core'
...
export async function commentBlock(
...
export async function pullBlock(node: GithubPullNode, ctx: StepCtx): Promise<StepResult> {
  const reach = reachOf(ctx)
