...
type Entry struct {
	...
	// omitzero, not omitempty: a directory always carries `children` — an empty folder is
	// `[]`, which is what the browser walks — while a file, whose slice is nil, carries none.
	Children []Entry `json:"children,omitzero"`
}
