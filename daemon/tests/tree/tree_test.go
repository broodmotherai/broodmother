...
// The browser walks `children` on every dir without looking first, so an empty folder must say
// `[]` in as many words — while a file carries no such key at all.
func TestAnEmptyFolderStillCarriesItsChildren(t *testing.T) {
	held := standing(t, map[string]string{"note.md": "x", "empty/.keep": "x"})
	if err := os.Remove(filepath.Join(held.Root, "empty", ".keep")); err != nil {
		t.Fatal(err)
	}
	entries, err := held.List()
	if err != nil {
	...
	if !strings.Contains(string(serialized), `"name":"empty","children":[]`) {
		t.Errorf("the empty folder did not carry []: %s", serialized)
	}
	if strings.Contains(string(serialized), `"name":"note.md","children"`) {
		t.Errorf("the file carried children: %s", serialized)
	}
}
