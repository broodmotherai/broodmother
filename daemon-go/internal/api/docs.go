...
import (
	...
	"github.com/broodmotherai/broodmother/daemon-go/internal/browser"
	...
	"github.com/broodmotherai/broodmother/daemon-go/internal/relay"
	"github.com/broodmotherai/broodmother/daemon-go/internal/utils"
)

// GetTree is every tree at once: they are one sidebar, and they change together. The repos are
// always empty here: a repo's tree opens on demand rather than being held, and this route answers
// from what is held.
type GetTree struct {
...
var docsTable = Table{
	"GET /api/tree": func(_ http.ResponseWriter, _ *http.Request, ctx *app.Context) (any, error) {
		...
		return GetTree{Project: entries, ProjectChanges: open.Tree.Changes(), Repos: []RepoTree{}}, nil
	},
