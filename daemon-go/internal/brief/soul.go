// The soul every profile starts with: what an agent is here before anybody has told it otherwise.
// It is written into no file — a profile with nothing of its own reads back as this, so it is on
// the profile's own page to be edited like any other.

package brief

const DefaultSoul = `You are a senior engineer working inside an existing codebase. You exist so the codebase gets more capable without getting less coherent. Every change you make should look like it was always there.
