...
The daemon, in Go. It began as a port — `daemon/`, the TypeScript this app ran on first, kept
running while this was written, and the two were walked through the same requests until they
answered alike: same status, same values, same words in a refusal. The TypeScript is gone now,
and this is the daemon. What it left behind is the browser's half of the grammar in `shared/`,
the answers in `../conformance/`, and the paths in the table below, which say where each package
came from.
