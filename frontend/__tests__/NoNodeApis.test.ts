 ...
 *  ours. Only the shapes that can reach the daemon's tree resolve: the three aliases, and a
 *  relative step taken from inside that tree. The aliases are the ones in `tsconfig.json`,
 *  and they have to stay in step with it — a specifier that stops resolving here does not
 *  fail, it just stops being read, which is the one way this guard can go quiet. */
