...
import { BLANK, isBrowsable } from '@shared/utils/browser'
import {
...
/**
 * The rule about where a browser tab may go is written twice on purpose: once in `shared`, where
 * the address bar can refuse an address before it is ever sent, and once in the desktop process,
 * which is the copy that actually decides what gets attached — a check the renderer could edit
