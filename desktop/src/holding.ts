...
export function holding(url: string, packaged = false, paint = windowPaint()): string {
  const html = `<!doctype html>
<meta charset="utf-8">
<title>broodmother</title>
<style>
  :root { color-scheme: ${paint.scheme} }
  body {
    display: flex; flex-direction: column; gap: .4rem;
    align-items: center; justify-content: center;
    height: 100vh; margin: 0;
    background: ${paint.holdingGround}; color: ${paint.holdingInk};
