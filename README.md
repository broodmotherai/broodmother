...
An app for a folder of markdown and the repositories that folder is about. The `.md` files on disk are the source of truth and git is the history. There is no database behind the documents, and nothing is written anywhere you could not open with an editor. What the app adds is what a folder cannot do for itself: agents that work in it, tasks that run on their own, boards it draws instead of typing, records it refuses to write carelessly, and shells with the checkout already open.

## Documents

![The editor open on this repository's own README](.github/assets/editor.png)

The sidebar is one project. The rows under its name are its notes. The rows under those, `api` and `broodmother` and `website`, are code repositories checked out inside it. There are as many as the notes cover, all open at once, each on whichever branch you left it on. The editor follows the file on disk, so a document changed behind the app's back reaches the page without anybody asking. A shell, a pull and an agent all count.
...
An agent is people-shaped. It has a name, a persona out of the project's `.personas/` folder, one conversation held with it for as long as it exists, and an attachments folder made at the same moment so it is in the tree from the first message. A reply arrives a word at a time. Every tool call it makes is listed as it lands, with what was read, what the shell said, and how many lines came back, so what an agent did is on the page rather than in a log somewhere. A socket closing does not stop a reply being written. A lid, a sleep and a reload all look alike from here, and none of them is somebody saying they were done.
...
A task is a flow the daemon runs: triggers on the left, the steps they set off after them. A trigger is a person pressing play, an interval, a time of day, a file changing, or GitHub. From GitHub that means an issue opened, a pull pushed to, a mention, or a branch's checks settling. A step is an agent errand, a shell command, a call out to a URL, a notification, or a comment or pull request written back to GitHub.
...
A `.canvas` is [JSON Canvas](https://jsoncanvas.org), the format Obsidian writes, so a diagram made here opens there. You get shapes, fills, edges that know which side they leave from and what they are labelled, and class boxes whose compartments are the name, the fields and the methods.

Both boards are documents the whole way down. The browser parses one to draw it and the daemon parses the same bytes before it writes them, and the two are held to the same corpus of recorded answers. A document one accepts and the other refuses is a document that opens and cannot be saved.

## Shells

![Two terminal panes side by side, each running an agent in the checkout](.github/assets/terminals.png)

Terminals are ptys the daemon opened, named by the tab watching them, and they outlive their socket the way a shell should outlive a shut lid. They open on the checkout you are looking at and split across the pane. The app can tell a shell sitting at a prompt from one that is busy, including a Claude Code session waiting to be told what next, which no process list can distinguish from one that is thinking.
...
The daemon and the site come up together, on ports asked of the OS rather than fixed, so a second
